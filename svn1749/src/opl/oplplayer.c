...
#include "opl_compat.h"   // [Arcade] in place of prboom-plus headers
...
// [Arcade] The cached GENMIDI lump, held PU_STATIC while the player is up.
static const byte *genmidi_lump = NULL;
...
static dboolean LoadInstrumentTable(void)
{
    ...
    // [Arcade] DoomLegacy's W_CacheLumpName is fatal on a missing lump.  The
    // lump stays PU_STATIC while the player may use it; see I_OPL_ShutdownMusic.
    if (!VALID_LUMP(W_CheckNumForName("GENMIDI"))
        || W_LumpLength(W_CheckNumForName("GENMIDI"))
           < (int) (strlen(GENMIDI_HEADER) + 2 * GENMIDI_NUM_INSTRS * sizeof(genmidi_instr_t)))
    {
        return false;
    }
    lump = (const byte*)W_CacheLumpName("GENMIDI", PU_STATIC);

    // Check header

    if (strncmp((const char *) lump, GENMIDI_HEADER, strlen(GENMIDI_HEADER)) != 0)
    {
        Z_ChangeTag((void *) lump, PU_CACHE);   // [Arcade]

        return false;
    }
    genmidi_lump = lump;   // [Arcade]
...
static void I_OPL_ShutdownMusic(void)
{
    if (music_initialized)
    {
        ...
        // [Arcade] Back to purgeable; the player no longer reads it.
        if (genmidi_lump)
            Z_ChangeTag((void *) genmidi_lump, PU_CACHE);
        genmidi_lump = NULL;
