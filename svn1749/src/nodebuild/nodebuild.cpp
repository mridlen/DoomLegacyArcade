...
// [Arcade] Built into the engine, where a progress bar on stderr is noise.
#ifndef NB_QUIET
#define NB_QUIET 0
#endif
#define STACK_ARGS
...
void FNodeBuilder::BuildTree ()
{
	...
	// [Arcade] progress goes to a terminal; the engine builds silently
	if (!NB_QUIET) fprintf (stderr, "   BSP:   0.0%%\r");
	HackSeg = DWORD_MAX;
	HackMate = DWORD_MAX;
	CreateNode (0, Segs.Size(), bbox);
	CreateSubsectorsForReal ();
	if (!NB_QUIET) fprintf (stderr, "   BSP: 100.0%%\n");
}
...
DWORD FNodeBuilder::CreateSubsector (DWORD set, fixed_t bbox[4])
{
	...
	if ((SegsStuffed & ~63) != ((SegsStuffed - count) & ~63))
	{
		...
		if (!NB_QUIET) fprintf (stderr, "   BSP: %3d.%d%%\r", percent/10, percent%10);
	}
