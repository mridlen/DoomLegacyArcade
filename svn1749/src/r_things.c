...
void R_DrawPlayerSprites (void)
{
    ...
    // [Arcade] A side-by-side view needs no correction here, and used to.
    // pspriteyscale is derived from the *projection* width now (fit_ref_width,
    // r_main.c), so a side-by-side weapon is drawn at the full screen scale in
    // a full height view -- arithmetically the full screen case, centerypsp
    // included.  While the scale came from the halved draw window the weapon
    // was drawn at half size and floated a quarter of the view above the
    // floor, and this is where that was compensated, by anchoring centery to
    // the bottom of the view instead of its middle.  That was the symptom:
    // the weapon was the wrong *size* as well, which is what showed on the
    // cabinet as a tiny gun next to the big one in top and bottom.  Fixing the
    // scale seats it without any anchor, and keeps the one pixel EN_doom_etc
    // nudge in centerypsp that the anchor discarded.
    //
    // The stacked halves keep their hand tuned 120 in R_DrawPSprite.  They are
    // the opposite case -- full scale in a half height view, so the weapon
    // overhangs the bottom and is cropped -- and that cropping is how the
    // cabinet has always looked.  See docs/arcade/multiplayer-views.md.
