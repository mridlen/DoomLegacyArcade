...
| `docs/arcade/attract.md` | Attract cycle, menu-over-attract backdrop, idle timeout, arcade death; the chase camera on record demos — the raycast placement, the collision spring, line-of-sight recovery, and why it must not touch the simulation | `D_AdvanceDemo`, `G_Idle_Timeout_Check`, `G_Arcade_Death_Check`, `P_MoveChaseCamera`, `P_Camera_Clear_Frac`, `tools/chasecam-test.py`, `tools/camstats.py` |
