...
(`M_SINLVL`, `M_JOIN`, `M_CHEATS`, `CREDIT2`, `SBOARMBL`, `M_GAMOVR`). `common/legacy.wad` is the
copy to edit: `tools/build.sh`/`build.ps1` stage it beside the binary, and the engine takes the
`legacy.wad` in the program directory ahead of every doomwaddir — so the old live copy in
`~/games/doom/` is shadowed once a build has staged one. Verify only the expected lumps differ
before committing. → `building.md`
...
- **The chase camera is measured, not watched.** `tools/chasecam-test.py` replays each demo with
  the camera off and on and requires identical `-synclog` output — the camera is a view and must
  never change the simulation, and it did (`attract.md`). It must **not** pass `-nodraw`:
  `R_Update_Chase_Camera` is called from `D_Display`, so under `-nodraw` the camera is never created
  and `make demotest` cannot see this class of bug at all. `tools/camstats.py` runs two builds over
  one demo through the engine's `-camlog` and tabulates how often the camera cannot see the player,
  the worst unbroken blind stretch, and direction reversals — which is what the jitter is. Read the
  >2-unit reversal figure, not the bare count: the bare one includes wobble nobody can see.
  → `attract.md`
...
| `docs/arcade/attract.md` | Attract cycle, menu-over-attract backdrop, idle timeout, arcade death; the attract freeze watchdog and `-demofreeze`; the chase camera on record demos — the raycast placement, the collision spring, line-of-sight recovery, and why it must not touch the simulation | `D_AdvanceDemo`, `G_Idle_Timeout_Check`, `G_Arcade_Death_Check`, `P_MoveChaseCamera`, `P_Camera_Clear_Frac`, `tools/chasecam-test.py`, `tools/camstats.py` |
...
| `docs/arcade/ultrawide.md` | 21:9 and 32:9 monitors, and portrait (narrower than 4:3): the engine size caps, the view fit and field of view, the wide draw sizes, the 2D and weapon aspect caps (`fdupx_fill`/`fdupy_fill`), the Video Modes aspect filter | `MAXVIDWIDTH`/`MAXVIDHEIGHT`, `VID_add_scaled_modes`, `windowedModes[]`, the `viewfit` block, `R_Init_TextureMapping` and the psprite scales in `r_main.c`, `HWR_DrawPSprite`, `vid.fdupy` or `vid.fdupx`, `vid_aspect` |
...
| `docs/arcade/crash-diagnostics.md` | The crash report (`legacyhome/crash.txt`), the class-list black box and `CLASS-LIST ANOMALY` lines; the Pi `PIT_FindTarget` segfault that prompted it, and the stale `target` in `P_HelpFriend` it found | `d_crash.c`, `D_Crash_*` call sites, anything that links a thinker into a class-list, `REFERENCE_COUNTING` |
...
| Crash report, class-list black box | `d_crash.c` entire (`D_Crash_Init` after `AU_Init` in `d_main.c`; `D_Crash_Set_Demo` in `G_DoPlayDemo`; `D_Crash_Class_Anomaly`/`D_Crash_Friend_Added` in `P_UpdateClassThink`/`P_MoveClassThink` (`p_tick.c`) and `PIT_FindTarget` (`p_enemy.c`)). Writes `legacyhome/crash.txt` |
...
| Screen shaders (OpenGL) | `sdl/ogl_shader.c` entire (`OGL_Shader_Present` from `OglSdl_FinishUpdate`, `OGL_Shader_Context_Lost` from `OglSdl_SetMode`); shaders in `sdl/shaders/`, embedded by `tools/glsl2c.py` into `sdl/ogl_shader_glsl.h`; `cv_grshader`/`gr_shader_status` (`hw_main.c`); the wipe's `ogl_read_front_hook` (`r_opengl.c`); the Shaders page `OGL_ShaderDef` (`m_menu.c`) |
| Invulnerability in OpenGL | `view_inverse_colormap` (`r_main.c`, set in `R_SetupFrame`), the `PF_Invert` quad at the end of `HWR_RenderPlayerView` (`hardware/hw_main.c`), `blend_param[]` (`hardware/r_opengl/r_opengl.c`) |
...
| Cabinet Link | `d_link.c` entire (the only file that may test `HAVE_LINK`: transport, routing, the UDP seal, the `SYNC` frame), `d_linkgame.c` (invites, on the game thread), `d_linksel.c` (Select Game Sync: `cv_link_gamesync`, `-linkselected` across the restart, `M_Link_Follow_Game` in `m_menu.c`; `-linkselectat S name` picks a game in a test; Copy Missing Wads, `cv_link_copywads`, IWADs into `wads/` beside the program, over the sync channel from kind 32, `-linkcorruptwad`) and `d_linkscore.c` (shared scores: manifests, demo pulls, the deferred apply; score cases `scores`/`scoreclear*`/`scorebad`/`scorerules`/`scorebusy`/`scorerestore*`, `-linkcmdat S "text"` (up to four) for console text at a time; the clear backup and `HS_Restore_Scores` are in `hs_stuff.c`). Hooks: `LK_Init` (`D_DoomMain`), `LK_Ticker` (`D_DoomLoop`), `LK_Shutdown` (`D_Quit_Save`); the pages are `CabinetLinkDef`/`LinkTextDef`/`LinkAllowDef` with keys in `M_Link_Page_Key` and every change through `LK_Setting_Set`/`LK_Allow_*`; the join screen's `M_Join_Remote_*` in `m_menu.c`; `D_Link_Connect` (`d_clisrv.c`); `LK_Net_Recv`/`LK_Net_Send` in `SOCK_Get`/`SOCK_Send` (`i_tcp.c`). Test with `tools/linktest.sh` (and `--selfcheck`); `-linkstatus` prints status every 2 s; `-linktest -linkautohost/-linkautojoin` drive invites headlessly and `-linkkeys` presses buttons on the page (`tools/linktest_kbd.py` types); `-linkcmdafter S "text"` types console text S seconds into a linked level; `-tictiming` measures a network game's smoothness |
