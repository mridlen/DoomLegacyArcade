...
expected lumps differ before committing. → `building.md`
...
| `docs/arcade/sound.md` | Sound channels and mixer slots, every way a sound stops early, the `-sndlog` tracer; the frame-cap bug that left finished sounds holding every channel, and missile sounds reading freed memory | `S_get_channel`, `S_UpdateSounds` and its call in `D_DoomLoop`, `S_StopXYZSound`, `mix_channel[]` in `sdl/i_sound.c` |
