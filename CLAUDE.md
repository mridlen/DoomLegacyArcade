...
| Cabinet Link | `d_link.c` entire (the only file that may test `HAVE_LINK`: transport, routing, the UDP seal, the `SYNC` frame), `d_linkgame.c` (invites, on the game thread) and `d_linkscore.c` (shared scores: manifests, demo pulls, the deferred apply; score cases `scores`/`scoreclear*`/`scorebad`/`scorerules`/`scorebusy`, `-linkcmdat S "text"` for console text at a time). Hooks: `LK_Init` (`D_DoomMain`), `LK_Ticker` (`D_DoomLoop`), `LK_Shutdown` (`D_Quit_Save`); the pages are `CabinetLinkDef`/`LinkTextDef`/`LinkAllowDef` with keys in `M_Link_Page_Key` and every change through `LK_Setting_Set`/`LK_Allow_*`; the join screen's `M_Join_Remote_*` in `m_menu.c`; `D_Link_Connect` (`d_clisrv.c`); `LK_Net_Recv`/`LK_Net_Send` in `SOCK_Get`/`SOCK_Send` (`i_tcp.c`). Test with `tools/linktest.sh` (and `--selfcheck`); `-linkstatus` prints status every 2 s; `-linktest -linkautohost/-linkautojoin` drive invites headlessly and `-linkkeys` presses buttons on the page (`tools/linktest_kbd.py` types); `-linkcmdafter S "text"` types console text S seconds into a linked level; `-tictiming` measures a network game's smoothness |
