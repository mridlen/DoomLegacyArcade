...
| `docs/arcade/crt-shaders.md` | The CRT post-process shaders in OpenGL: the libretro shaders and their conventions, the downsample to game-sized scanlines, the wipe hook, re-entry | `sdl/ogl_shader.c`, `sdl/crt/`, `tools/glsl2c.py`, `cv_grshader`, `ogl_read_front_hook` |
...
| CRT shaders (OpenGL) | `sdl/ogl_shader.c` entire (`OGL_Shader_Present` from `OglSdl_FinishUpdate`, `OGL_Shader_Context_Lost` from `OglSdl_SetMode`); shaders in `sdl/crt/`, embedded by `tools/glsl2c.py` into `sdl/ogl_crt_glsl.h`; `cv_grshader`/`gr_shader_status` (`hw_main.c`); the wipe's `ogl_read_front_hook` (`r_opengl.c`); the Shaders page `OGL_ShaderDef` (`m_menu.c`) |
